<?php
session_start();
require_once("../controller/controlloginguru.php");
...
error_reporting(0);
	...
	$controll = new loginGuru();
	$data = $controll->logincontrolGuru($idp, $sandip, $deviceType, $parts[count($parts)-1], $parts[0]." ".$parts[1], $mark1, $ip);
	echo json_encode($data);
	foreach($data as $key => $value) {
		$_SESSION['riduserp'] = $value['riduserp'];
		$_SESSION['userp'] = $value['userp'];
		$_SESSION['ridgurup'] = $value['ridgurup'];
