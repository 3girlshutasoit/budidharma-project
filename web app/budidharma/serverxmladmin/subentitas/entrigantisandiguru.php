...
$server->register("databasequery",
					array(	"sandilama"		=> "xsd:string",
							"sandibaru"		=> "xsd:string",
							"idusergurup"	=> "xsd:string"
					),
...
function databasequery($sandilama, $sandibaru, $idusergurup){
	...
	$sql  = "
		select 	  respon,
				  pesan
		from subentitas.fgantisandiguru(sandilama 	:=:sandilama,
							   			sandibaru 	:=:sandibaru,
	...
	$cmd = $db->prepare($sql);
	$cmd->bindValue(':sandilama', $sandilama, PDO::PARAM_STR);
	$cmd->bindValue(':sandibaru', $sandibaru, PDO::PARAM_STR);
	$cmd->bindValue(':idusergurup', $idusergurup, PDO::PARAM_STR);
	$cmd->execute();
	$ok = $cmd->fetchAll();
