...
$server->wsdl->addComplexType(
	...
	array(
		...
		"sandio"   => array("name" => "sandio", "type" => "xsd:string"),
         )
);  
...
function databasequery($idr){
	...
	$sql  = "
		select 	  respon,
				  pesan,
				  sandio
		from subentitas.fresetsandiguru(idr :=:idr)
	";

 

	$cmd = $db->prepare($sql);
	$cmd->bindValue(':idr', $idr, PDO::PARAM_STR);
	$cmd->execute();
	$ok = $cmd->fetchAll();
