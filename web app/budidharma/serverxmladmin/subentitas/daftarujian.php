...
$server->wsdl->addComplexType(
	...
	array(
		...
		"nama_bulanujian"	=> array("name" => "nama_bulanujian", "type" => "xsd:string"),
...
function databasequery($tap, $kup){
	$conn = new db();
	$db = $conn->dbconn();
	
	$sql  = "
		select *
		from subentitas.ftabdaftarujian	(tap	:=:tap,
							 		 	 kup   	:=:kup)
	";
		...
		if(empty($ok)){
			...
			foreach($ok as $field){
				$data[] = array(
							...
							"nama_bulanujian"	=> $field['nama_bulanujian'],
