...
$server->wsdl->addComplexType(
	...
	array(
		...
		"id_pel"		=> array("name" => "id_kelas", "type" => "xsd:string"),
...
function databasequery($kelasp, $mapelp){
	$conn = new db();
	$db = $conn->dbconn();
	
	$sql  = "
		select *
		from subentitas.ftabkelasmapel	(kelasp		:=:kelasp,
							 		 	 mapelp   	:=:mapelp)
	";
	...
	$cmd->bindValue(':mapelp', $mapelp, PDO::PARAM_STR);
	$cmd->execute();
	$ok = $cmd->fetchAll();
